...
export const Employess = {
    Get_Id: async (id: string): Promise<any> =>
        await http.get(`${EnvConfig.NEXT_PUBLIC_HOST}${EnvConfig.NEXT_PUBLIC_EMPLOYESS}/${id}`, { headers: { 'x-access-token': Token } }),
    Get_All: async () =>
        await http.get(`${EnvConfig.NEXT_PUBLIC_HOST}${EnvConfig.NEXT_PUBLIC_EMPLOYESS}`, { headers: { 'x-access-token': Token } }),
    Create: async (body: Update_User_Type): Promise<any> =>
        await http.post(`${EnvConfig.NEXT_PUBLIC_HOST}${EnvConfig.NEXT_PUBLIC_EMPLOYESS}`, body, { headers: { 'x-access-token': Token } }),
    Update: async (id: string, body: Update_User_Type): Promise<any> =>
        await http.put(`${EnvConfig.NEXT_PUBLIC_HOST}${EnvConfig.NEXT_PUBLIC_EMPLOYESS}/${id}`, body, { headers: { 'x-access-token': Token } }),
    Delete: async (id: string) =>
        await http.delete(`${EnvConfig.NEXT_PUBLIC_HOST}${EnvConfig.NEXT_PUBLIC_EMPLOYESS}/${id}`, { headers: { 'x-access-token': Token } })
};
